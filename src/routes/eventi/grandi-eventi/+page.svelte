<script>
	...
	$: filtered = data.events.filter((e) => e.type === 'grandi-eventi');
</script>
...
>
	<div class="mx-auto max-w-6xl">
		<h1 class="mb-12 text-center text-4xl font-extrabold tracking-tight text-zinc-900">
		...
		{#if filtered.length === 0}
			<div
				class="animate-fadeIn mx-auto max-w-3xl rounded-2xl border border-zinc-100 bg-white p-12 text-center text-zinc-700 opacity-0 shadow-xl transition-all duration-300"
			>
				<div
		...
		{:else}
			<div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
				{#each filtered as event, i (event.id)}
					<div
						class="group animate-fadeIn animation-delay-[i*100] flex flex-col overflow-hidden rounded-2xl bg-white opacity-0 shadow-lg transition-transform duration-300 hover:-translate-y-2 hover:shadow-2xl"
					>
						<a href={`/eventi/${event.id}`} class="block flex-1">
							...
							<div class="flex flex-col p-6">
								<div class="mb-4 flex flex-wrap items-center gap-3 text-xs text-zinc-700">
									<!-- Data -->
